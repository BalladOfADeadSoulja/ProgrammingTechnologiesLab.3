...
void initMainWindow(HWND hWnd) {
	...
	HWND editRowCount = CreateWindowA("edit", "", WS_VISIBLE | WS_CHILD | ES_NUMBER | WS_BORDER, 150, 35, 60, 20, hWnd, NULL, NULL, NULL);
	HWND editColumnCount = CreateWindowA("edit", "", WS_VISIBLE | WS_CHILD | ES_NUMBER | WS_BORDER, 150, 65, 60, 20, hWnd, NULL, NULL, NULL);

	CreateWindow(L"static", L"[ Vector/Scalar info ]", WS_VISIBLE | WS_CHILD, 50, 95, 150, 20, hWnd, 0, 0, 0);

	HWND radioVector = CreateWindowA("button", "Vector Size:", WS_VISIBLE | WS_CHILD | BS_AUTORADIOBUTTON, 10, 125, 100, 20, hWnd, (HMENU)BTN_RADIO_VECTOR, NULL, NULL);
	HWND radioScalar = CreateWindowA("button", "Scalar:", WS_VISIBLE | WS_CHILD | BS_AUTORADIOBUTTON, 10, 155, 100, 20, hWnd, (HMENU)BTN_RADIO_SCALAR, NULL, NULL);
	HWND editVectorSize = CreateWindowA("edit", "", WS_VISIBLE | WS_CHILD | ES_NUMBER | WS_BORDER, 120, 125, 80, 20, hWnd, NULL, NULL, NULL);
	HWND editScalarValue = CreateWindowA("edit", "", WS_VISIBLE | WS_CHILD | ES_NUMBER | WS_BORDER, 120, 155, 80, 20, hWnd, NULL, NULL, NULL);

	HWND buttonCalc = CreateWindowA("button", "Calculate", WS_VISIBLE | WS_CHILD, 10, 185, 100, 20, hWnd, (HMENU)BTN_CALCULATE, NULL, NULL);
	...
	if (stateInfo.isVector) { //Если используется вектор
		...
		for (int i = 0; i < mtx.size(); ++i) {
			...
			for (int j = 0; j < mtx[i].size(); ++j) {
				answer[i][j] = CreateWindowA("edit", std::to_string(mtx[i][j] * stateInfo.valueScalar).c_str(),
					WS_VISIBLE | WS_CHILD | ES_NUMBER | WS_BORDER,
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrev, LPSTR szCmdLine, int nShowCmd) {
	...
	WNDCLASSEX classMainWindow = createWindowsClass(hInst, L"classMainWindow", callbackMainWindow);
	WNDCLASSEX classSubWindow = createWindowsClass(hInst, L"classSubWindow", callbackSubWindow);
